...
export class ServiceService {
  ...
  async getService(categoryId: number): Promise<ResponseDto<any>> {
    try {
      const data = await this.serviceRepo.repo.find({ where: { status: ServiceStatus.ACTIVE, categoryId }, order: { id: 'ASC' } });
  ...
  async updateService(id: number, body: UpdateServiceDto): Promise<ResponseDto<any>> {
    try {
      ...
      const { name, sourceAddress, price, rate} = body;

      await this.serviceRepo.repo.update({ id }, { name, sourceAddress, price, rate });
